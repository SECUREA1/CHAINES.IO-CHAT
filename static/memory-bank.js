(function(){
  const CACHE_KEY='chaines_memory_v2', MIGRATION_VERSION=2;
  const safeLegacyKeys=['chaines_profile_pic','chaines_messages','chaines_rewards_v1','chaines_marketplace_likes','chaines_marketplace_contacts','chaines_marketplace_dating_matches','chaines_theme','chaines_ws_url','chaines_autodelete','chaines_verified_users','mixer_current_chain','mixer_current_currency'];
  const sensitiveLegacyKeys=['chaines_obs_key','chaines_session_validation'];
  let user=null, namespaces={}, subscribers=new Map(), pending=new Set(), timer=null, initialized=false, channel=null;
  const readCache=()=>{ try{return JSON.parse(localStorage.getItem(CACHE_KEY)||'{"version":2,"users":{}}');}catch{return {version:2,users:{}};} };
  const writeCache=(cache)=>{ localStorage.setItem(CACHE_KEY, JSON.stringify(cache)); };
  function userCache(){ const c=readCache(); c.version=2; c.users=c.users||{}; if(user){ c.lastAuthenticatedUserId=String(user.id); c.users[user.id]=c.users[user.id]||{username:user.username,cachedAt:0,namespaces:{}}; } return c; }
  ...
  async function syncOne(ns){ const value=namespaces[ns] || {}; const res=await fetch(`/api/memory/${encodeURIComponent(ns)}`,{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({schemaVersion:value.schemaVersion||2,data:value.data!==undefined?value.data:value})}); if(!res.ok) throw new Error(`memory sync ${ns} ${res.status}`); return res.json(); }
  function schedule(ns){ pending.add(ns); saveLocal(); clearTimeout(timer); timer=setTimeout(()=>window.MemoryBank.flush(),450); }
  async function migrateLegacy(){ if(!user) return; const marker=`legacy-migration-v${MIGRATION_VERSION}`; if(namespaces[marker]) return; const backup={}; const preferences={}, wallet={}; for(const key of safeLegacyKeys){ const v=localStorage.getItem(key); if(v==null) continue; backup[key]=v; if(key==='chaines_theme') preferences.theme=v; else if(key==='mixer_current_chain') wallet.selectedChain=v; else if(key==='mixer_current_currency') wallet.selectedCurrency=v; }
    ...
    if(localStorage.getItem('chaines_rewards_v1')) { try{ window.MemoryBank.set('rewards', JSON.parse(localStorage.getItem('chaines_rewards_v1'))); }catch{} }
    localStorage.setItem(`${CACHE_KEY}_legacy_backup_${user.id}`, JSON.stringify({at:Date.now(),backup,sensitiveSkipped:sensitiveLegacyKeys}));
    window.MemoryBank.set(marker,{completedAt:Date.now(),sensitiveSkipped:sensitiveLegacyKeys});
  }
  window.MemoryBank={
    ...
    set(ns,value){ namespaces[ns]={schemaVersion:2,updatedAt:Date.now(),data:value}; schedule(ns); emit(ns); },
